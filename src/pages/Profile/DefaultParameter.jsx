...
// Reset axios interceptors to default state
...
const CreateProfile = () => {
  ...
  useEffect(() => {
    ...
    console.log("Auth User:", authUser); // Debug authUser
    if (!authUser || !authUser.accessToken || authUser.role !== "Admin") {
      // toast.error('Only users with the "Admin" role can modify default parameters. Please log in with appropriate credentials.');
      navigate("/login");
  ...
  const fetchDefaultParameters = async () => {
    ...
    try {
      const authUser = JSON.parse(sessionStorage.getItem("authUser"));
      console.log("Fetching with token:", authUser.accessToken); // Debug token
      const response = await axiosInstance.get(`${BASE_URL}/api/default/default-parameters/`, {
        headers: {
      ...
      console.log("Initial Response Data:", responseData); // Debug initial data
      // Fallback parsing if data is undefined but response contains data
      if (typeof responseData === "undefined" && response.status === 200) {
        try {
          responseData = JSON.parse(response.config.data || "{}"); // Fallback to config data if available
          console.log("Parsed Response Data from Config:", responseData);
        } catch (parseError) {
          responseData = response.data; // Revert to original if parsing fails
      ...
      console.log("Processed Response Data:", responseData); // Debug processed data
      ...
      // toast.error('Faild to Fetch Date');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.put(
        `${BASE_URL}/api/default/default-parameters/`,
        {
