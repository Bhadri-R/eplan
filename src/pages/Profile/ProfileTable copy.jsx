...
import { Row, Col, Card, CardBody, FormGroup, Input, Button, Table, Modal, ModalHeader, ModalBody, ModalFooter } from 'reactstrap';
...
const ProfileTable = ({ refresh }) => {
    ...
    const fetchProfileData = async () => {
        try {
            ...
            const normalizedData = Array.isArray(profileData) ? profileData : [profileData];
        ...
        <Card>
            <CardBody>
                            ...
                            {currentItems && currentItems.length > 0 ? (
                                currentItems.map((profile, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>
                                            <Button color="white" className='me-2' style={{ color: '#fabe13' }} onClick={() => handleViewProfile(profile)}>
                                                <FaEye />
                                            </Button>
                                            <Button color="white" className='me-2' style={{ color: '#28a745' }} onClick={() => handleEditProfile(profile)}>
                                                <MdEditSquare />
                                            </Button>
                                        </td>
                                    </tr>
                            ...
                            <div>
                                <FormGroup>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => setSelectedProfile({ ...selectedProfile, clientName: e.target.value })}
                                    />
                                </FormGroup>
                                <FormGroup>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => setSelectedProfile({ ...selectedProfile, clientAge: e.target.value })}
                                    />
                                </FormGroup>
                                <FormGroup>
                                <label className="form-label">Client Retirement Age:</label>
                                    <Input
                                        ...
                                        onChange={(e) => setSelectedProfile({ ...selectedProfile, clientRetirementAge: e.target.value })}
                                    />
                                </FormGroup>
                ...
                {viewModal && selectedProfile && (
                    <div className={`modal modal-overlay-editview fade show d-block`} tabIndex="-1" role="dialog">
                        <div className="modal-dialog modal-dialog-centered">
                            <div className="modal-content">
                                <div className="modal-header">
                                    <h5 className="modal-title">{viewModal ? "View Profile" : "Edit Profile"}</h5>
                                    ...
                                    {selectedProfile && (
                                        <>
                                            <div className="mb-3">
                                                ...
                                                <input type="text" className="form-control" value={selectedProfile.clientName || ""} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Client Age:</label>
                                                <input type="text" className="form-control" value={selectedProfile.clientAge || ""} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Client Retirement Age:</label>
                                                <input type="text" className="form-control" value={selectedProfile.clientRetirementAge || ""} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Client Mobile:</label>
                                                <input type="text" className="form-control" value={selectedProfile.clientMobile || ""} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Spouse Name:</label>
                                                <input type="text" className="form-control" value={selectedProfile.spouseName || "-"} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Spouse Age:</label>
                                                <input type="text" className="form-control" value={selectedProfile.spouseAge || "-"} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Spouse Retirement Age:</label>
                                                <input type="text" className="form-control" value={selectedProfile.spouseRetirementAge || "-"} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Spouse Mobile:</label>
                                                <input type="text" className="form-control" value={selectedProfile.spouseMobile || "-"} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Children:</label>
                                                {selectedProfile.children && selectedProfile.children.length > 0 ? (
                                                    selectedProfile.children.map(child => (
                                                        <input key={child.id} type="text" className="form-control mb-1" value={`Name :  ${child.name} ; Age : ${child.age}`} disabled />
                                                    ))
                                                ) : (
                                                    <input type="text" className="form-control" value="No Children" disabled />
                                                )}
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Client Email:</label>
                                                <input type="text" className="form-control" value={selectedProfile.clientEmail || "-"} disabled />
                                            </div>
                                            <div className="mb-3">
                                                <label className="form-label">Spouse Email:</label>
                                                <input type="text" className="form-control" value={selectedProfile.spouseEmail || "-"} disabled />
                                            </div>

                                            {selectedProfile.profile && (
                                                <>
                                                    <div className="mb-3">
                                                        <label className="form-label">Profile Source:</label>
                                                        <input type="text" className="form-control" value={selectedProfile.profile || "-"} disabled />
                                                    </div>
                                                    <div className="mb-3">
                                                        <label className="form-label">Profile Amount:</label>
                                                        <input type="text" className="form-control" value={selectedProfile.profile || "-"} disabled />
                                                    </div>
                                ...
                                <div className="modal-footer">
                                    <button className="btn btn-primary" onClick={() => setViewModal(false)}>Close</button>
                                </div>
                            </div>
                        </div>
                    </div>
