...
const Createliabilities = ({ fetchLiabilitiesData }) => {
  ...
  const [errors, setErrors] = useState([{}]); // ✅ Change from {} to []
  ...
  const handleRemoveIncome = (index) => {
    ...
    setLiabilitiesDetails(newLiabilitiesDetails); // ✅ Correct state update
  ...
  const validateMonthlyPayment = (value) => {
    if (!value) return "Monthly balance is required";
    if (isNaN(value)) return "Monthly balance must be a number";
    return "";
  };
  ...
  //     const newErrors = liabilitiesDetails.map((detail, index) => {
  //         const detailErrors = {};
  ...
  const validateForm = () => {
    ...
    const newErrors = liabilitiesDetails.map((detail) => {
      ...
      detailErrors.monthlyPayment = validateMonthlyPayment(
        detail.monthlyPayment
      );
      detailErrors.currentBalance = validateCurrentBalance(
        detail.currentBalance
      );

      // ✅ Exclude 'description' from validation
      if (
        detailErrors.source ||
        detailErrors.type ||
  ...
  const handleCreateliabilities = async (e) => {
    ...
    try {
      ...
      const successMessage =
        response.message || "Liability details created successfully!";
          ...
          {liabilitiesDetails.map((detail, index) => (
            <Row key={index}>
              <Col md={4}>
                <FormGroup>
                  ...
                  <Input
                    ...
                    disabled={loadingClient} // ✅ Disable dropdown while loading
                  >
              ...
              <Col md={4}>
                <FormGroup>
                  <Label>
                    Monthly Balance<span class="text-danger"> *</span>{" "}
                  </Label>
                  <Input
                    type="text"
                    name="monthlyPayment"
                    ...
                    onChange={(e) => handleChange(e, index)}
                    invalid={!!errors[index]?.monthlyPayment} // ✅ Add this line
                  />
                  <FormFeedback className="text-capitalize">
              ...
              <Col md={4}>
                <FormGroup>
                  ...
                  <Input
                    ...
                    onChange={(e) => handleChange(e, index)}
                    invalid={!!errors[index]?.currentBalance} // ✅ Add this line
                  />
                  <FormFeedback className="text-capitalize">
              ...
              <Col md={4}>
                <FormGroup>
                  ...
                  <Input
                    ...
                    onChange={(e) => handleChange(e, index)}
                    invalid={!!errors[index]?.description} // ✅ Add this line
                  />
                  {/* <FormFeedback className="text-capitalize">{errors[index]?.description}</FormFeedback> */}
                </FormGroup>
              </Col>
